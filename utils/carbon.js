...
export class Upload {
  ...
  next() {
    ...
    if (this.chunkCount > 1) {
      console.log(
        `'${this.name}' uploading ${this.chunkNumber} of ${
          this.chunkCount
        } chunks.`
      );
    }
    ...
    chunk
      .send(this)
      .then(this.onChunkUploaded.bind(this), this.onChunkFailed.bind(this));
  }
  onChunkFailed(chunk) {
    if (this.debug) {
      console.log(
        "Chunk failed, auto retrying in 1s. " + this.retryCount + " of 3."
      );
    }
    ...
    if (options.authorization) {
      if (typeof options.authorization == "string") {
        ...
        xhr.setRequestHeader(
          "Authorization",
          "Bearer " + options.authorization.token
        );
      }
    }
